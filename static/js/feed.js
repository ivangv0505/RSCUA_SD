...
function conectarSocket() {
    if (typeof io !== 'undefined') {
        ...
        // Escuchar notificaciones con el nombre de evento correcto
...
async function actualizarBadge() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (badge) {
                badge.innerText = noLeidas > 0 ? (noLeidas > 99 ? "99+" : noLeidas) : "";
    ...
    return `
        <div class="publicacion" id="post-${post.id}">
            ...
            <div id="seccion-comentarios-${post.id}" class="seccion-comentarios">
                <div class="lista-comentarios" id="lista-comentarios-${post.id}"></div>
...
// Resto de funciones (crearPublicacion, modals, logout...). No se han modificado
...
async function confirmarEliminar() {
    ...
    await fetch(`${API_URL}/publicaciones/${postAEliminarId}`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${localStorage.getItem("token")}` },
    });
    const elem = document.getElementById(`post-${postAEliminarId}`);
    if (elem) elem.remove();
    cerrarModalEliminar();
}
